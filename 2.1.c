...
int copyData(siswa Data2[], siswa Data[], int n){
	...
	for(i=0; i<n; i++){
		...
		strcpy(Data[i].Alamat, Data2[i].Alamat);
...
int alokasi(siswa Data2[], int n){
	...
	for(i=0; i<n; i++){
		...
		fflush(stdin);
		printf("%-10s", "Alamat"); printf(": ");
		scanf("%s", &Data2[i].Alamat);
		fflush(stdin);
...
void menu(siswa Data[], int n){
	...
	switch(pilihan){
		case 1:
			insertion(Data, n, ascDesc);
			break;
		case 2:
			selection(Data, n, ascDesc);
			break;
	}
}
...
int insertion(siswa Data[], int n, int ascDesc){
...
int selection(siswa Data[], int n, int ascDesc){
	
	int i,j, x;
    for(i=0; i<n-1; i++){
        x = i;
        for(j = i+1; j<n; j++){
        	if(Data[j].NO < Data[x].NO && ascDesc == 1 || Data[j].NO > Data[x].NO && ascDesc == 2){
        		x = j;
			}
		}
        tukar(&Data[i], &Data[x]);
...
void tampil(siswa Data[], int n){
	...
	for(i=0; i<n; i++){
		printf("\n");
		printf("%d ", Data[i].NO); printf("%s ", Data[i].Nama); printf("%s ", Data[i].Alamat); printf("%d ", Data[i].Nilai);
	}
	printf("\n");
}
